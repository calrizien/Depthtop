...
struct RealityKitPreviewView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            RealityView { content in
                ...
                // Call updates directly but avoid modifying SwiftUI state
                updateWindowEntitiesDeferred(content: content)
                updateCameraPosition(content: content)
            }
            .background(Color.black)
            .onReceive(Timer.publish(every: 0.1, on: .main, in: .common).autoconnect()) { _ in
                // Only trigger updates when immersive space is closed
    ...
    private func updateWindowEntitiesDeferred(content: RealityViewCameraContent) {
        // Stop all processing when immersive space is active
        if appModel.immersiveSpaceState != .closed {
            return
        }
        ...
        // Only log when window count changes
        ...
        for (index, capturedWindow) in capturedWindows.enumerated() {
            ...
            if let existingEntity = entityStorage.windowEntities[windowID] {
                ...
                // Mark that texture needs to be updated later (not during view update)
                if capturedWindow.texture != nil {
                    print("🎨 Texture available, will update on next cycle...")
                    // Don't update texture immediately - let the regular update cycle handle it
                } else {
                    print("⚠️ No texture available for window, using placeholder material")
                }
            }
        }
    ...
    private func updateWindowTexture(_ entity: Entity, with window: CapturedWindow) {
        ...
        // Update texture synchronously if possible
        guard let modelEntity = entity as? ModelEntity,
              let texture = window.texture else { return }
        
        // For now, just update with a simple colored material to avoid async issues
        // The proper texture will be applied when the async operation completes
        if let existingMesh = modelEntity.model?.mesh {
            // Create a temporary tinted material to show update is happening
    ...
    private func updateCameraPosition(content: RealityViewCameraContent) {
        // Stop camera updates when immersive space is active
        if appModel.immersiveSpaceState != .closed {
            return
        }
